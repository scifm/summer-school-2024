...
# ╔═╡ e8cfc771-b8b0-4999-bf83-0b47296fd7fc
using DataFrames, CSV, Downloads

# ╔═╡ 739cbe4a-8eb6-4f75-959a-12bf138b8052
using WGLMakie # Plotting Library https://docs.makie.org/v0.21/

# ╔═╡ a3d46331-1d3d-44fd-8761-91a6584451c5
using Optimization, OptimizationOptimJL

# ╔═╡ 3d9ca43e-d4ca-4c6a-8761-da7e881b5ca6
using LossFunctions: HuberLoss

# ╔═╡ 3bba0651-ba2e-43c1-b8a1-44eb0ebd0fb9
using ForwardDiff
...
![Kaplan Scaling Laws](https://github.com/scifm/summer-school-2024/blob/main/img/kaplan_scaling.png?raw=true)
...
![Compute Optimal Scaling](https://github.com/scifm/summer-school-2024/blob/main/img/compute_optimal_scaling.png?raw=true)
...
![Scaling Up Chinchilla](https://github.com/scifm/summer-school-2024/blob/main/img/chinchilla_model.png?raw=true)
...
# ╔═╡ 4f1a0c0c-2c5a-4a6c-9b40-ee54d05b7fc4
md"""
We're going to use scaling data from [Scaling Data-Constrained Language Models](https://arxiv.org/abs/2305.16264), specifically the data from their  [parametric_fit.ipynb](https://colab.research.google.com/drive/1tYIfsmOMoz4dZ_JiVp998vZiMhRqSQrf?usp=sharing#scrollTo=LLCG0n7lbUNF) notebook.
"""
...
# ╔═╡ 27a23274-58b0-40fc-bf5e-0247b9627e06
describe(df, :min, :max, :mean, :median)

# ╔═╡ 0902d41c-4fe2-4a5d-9838-6287bb81180f
function plot_raw_data(model_size, data_size, loss)
	f = Figure()
	ax = Axis(f[1, 1];
	...
	C = @. 6 * model_size * data_size
	h = scatter!(ax, C, model_size;
		color=loss,
		colorscale=log10,
		colormap=:viridis,
		colorrange=(0.5, 40),
...
# ╔═╡ e2e2ca82-e373-4ef0-b96e-da4deb483dfd
plot_raw_data(df.model_size, df.total_tokens, df.loss) |> first
...
|:-----------:|:-------|:--------:|
| ``N``   	| Number of *non-embedding* parameters | Covariate |
| ``D`` 	| Number of tokens seen by the model during training | Covariate |
| ``L``     | Loss of the model for ``N`` and ``D`` | Response |
| ``A``, ``\alpha`` | Scaling parameters for model size | Parameter |
| ``B``, ``\beta`` | Scaling parameters for data size | Parameter |
| ``E`` | Intrinsic Entropy of the Data | Parameter |

We're going to follow the procedure used by [Hoffman et al.](https://arxiv.org/abs/2203.15556) to fit the above function. Specfically, we're going to minimize the following:

``
\min\limits_{a, b, e, \alpha, \beta} \sum_i Huber_{\delta} \left(
	LSE(a - \alpha \log N_i, b - \beta \log D_i, e) - \log L_i
\right)
``

To avoid ``\log(-1)`` errors, we do the following transformation: ``A = \exp(a)``, ``B = \exp(b)`` and ``E = \exp(e)``.

The actual optimization, uses the L-BFGS algorithm from [Optim.jl](https://julianlsolvers.github.io/Optim.jl/stable/) using [Optimization.jl](https://docs.sciml.ai/Optimization/stable/) with forward-mode automatic differentiation via [ForwardDiff.jl](https://github.com/JuliaDiff/ForwardDiff.jl).
"""

# ╔═╡ 563bc8bf-23b8-40d5-85f2-782794df3a54
begin
"""
	l = lse(x...)

Computes the [log-sum-exp](https://en.wikipedia.org/wiki/LogSumExp) of it's inputs
"""
lse(x...) = log(sum(exp, [x...]))
...
function hoffmann_scaling(x, p)
	...
	# Compute Huber Loss over the data
	fit_loss = 0
	lossfn = HuberLoss(1e-3) # Set δ=1e-3, per Hoffman et. al.
	for (N, D, l) in zip(model_size, data_size, loss)
		loss_lse = lse(a - α * log(N), b - β * log(D), e)
		fit_loss += lossfn(loss_lse - log(l))
	end
	return fit_loss
end
...
# ╔═╡ 59c6227a-b656-4d6d-8ffb-295d9672a75f
md"""
## Initial Grid Search
To avoid getting stuck in a local minima, we're first going to perform a grid search over "resonable" values for ``\alpha``, ``\beta``, a, b & e.
"""

# ╔═╡ a2219f82-ea42-4964-b2e9-fb3f9be3846c
...
# ╔═╡ 609bccd9-438b-4490-ab67-40d0c3a1d189
md"""
We're going to pretend training stopped after the first epoch, but in reality [Muennighoff et. al.'s](https://arxiv.org/abs/2305.16264) trained for multiple epochs. To fit on the total number of tokens the model saw change `df.unique_tokens` => `df.total_tokens` (The notebook will update automatically)
"""

# ╔═╡ c4e03d21-1bbb-4093-93b5-2f66b275102a
p = (; model_size=df.model_size, data_size=df.unique_tokens, loss=df.loss);
...
md"""
## Fitting Parameters with L-BFGS
We're using [Optimization.jl](https://docs.sciml.ai/Optimization) from Julia's [SciML](https://sciml.ai/) ecosystem, so the setting up and fitting the coefficients is pretty simple (and super customizable). For a deeper dive, see [Solving the Rosenbrock Problem in >10 Ways](https://docs.sciml.ai/Optimization/stable/examples/rosenbrock/).

Afterwards, we're left with our fitted parameters:
"""
...
"""
	N_opt = compute_optimal_model(C; p)

Returns the optimal model size for a given compute budget (C) in FLOPs
"""
function compute_optimal_model(C; p)
	(; A, α, B, β, E) = p
	G = ((α * A) / (β * B)) ^ (1/(α + β))
...
# Helper for getting a constant-ratio range of values
logrange(l, h; length=10) = map(exp, range(log(l), log(h); length))

function plot_compute_optimal(model_size, data_size, loss; p)
	# Set the limits of our predictions
	C_opt = logrange(1e15, 2e22; length=100)
	N_plt = logrange(1e6, 2e10; length=100)

	# Setup a figure and axis to plot onto
	f = Figure()
	ax = Axis(f[1, 1];
		limits=(extrema(C_opt), extrema(N_plt)),
		xscale=log10, yscale=log10,
		xlabel="Compute [FLOP]",
		ylabel="Model Size",
	)
	...
	# Add IsoLoss Contours
	(; A, α, B, β, E) = p
	loss_fit = map(Iterators.product(C_opt, N_plt)) do (c, n)
		d = c/(6*n)
		(A/(n^α)) + (B/(d^β)) + E
	end
	contour!(ax, C_opt, N_plt, loss_fit;
		levels=logrange(0.5, 40; length=25),
		colorrange=h.colorrange,
		colormap=h.colormap,
		colorscale=h.colorscale,
		label="IsoLoss Contours",
	)
...
md"""
## Limits of Scaling
Unfortunatly, scale isn't everything. Langauge has an intrinsic entropy, beyond which futher improvement isn't possible. [Shun-ichi Amari and Noboru Murata](https://doi.org/10.1162/neco.1993.5.1.140) provide an analytical derivation for this limit assuming both a asymptotically corrent and incorrect models.

``
H = \sum_{x \in \chi} p(x)\log p(x)
``
...
On the plus side, this limit tends to be more theroetical than practical. Plugging in ``N_{opt}`` and ``D_{opt}`` into ``L``, we can estimate the loss of a compute-optimal model for a given budget.
"""

# ╔═╡ 32952bfd-3751-404f-8e75-c34541402ed0
function compute_optimal_loss(C; p)
	(; A, α, B, β, E) = p
    G = ((α * A) / (β * B))^(1 / (α + β))
    a = β / (α + β)
    b = α / (α + β)
...
begin
function plot_compute_optimal_loss(C, loss, p)
	C_fit = logrange(1e15, 1e30; length=100)
	loss_fit = map(c -> compute_optimal_loss(c; p), C_fit)
	f = Figure()
	ax = Axis(f[1, 1];
		limits=(extrema(C_fit), (1, nothing)),
		xscale=log10, yscale=log10,
		xlabel="Compute Budget [FLOPS]",
		ylabel="Loss",
	)
	lines!(ax, C_fit, loss_fit; color=:red, label="Compute-Optimal Loss")
	scatter!(ax, C, loss; label="Emperical")
	hlines!(ax, p.E; label="Fitted Minimal Loss", color=:black)
	axislegend(ax)
...
md"""
Main takeaways:
1. The Compute-Optimal Loss is (generally) the lowest loss observed for a given compute budget
...
![model shape](https://github.com/scifm/summer-school-2024/blob/main/img/model_shape.png?raw=true)
...
Cool stuff right? Here are some more things to try out:
1. Try [Kaplan et. al's](https://arxiv.org/abs/2001.08361) model (Drop ``E`` )
2. Try [Muennighoff et. al.'s](https://arxiv.org/abs/2305.16264) method to account for multi-epoch training
3. Fit models to your own scaling runs 🤔

"""

# ╔═╡ 2a83f2b0-4562-4720-9ba5-ca7e9432f637
md"""
# Julia ![](https://avatars.githubusercontent.com/u/743164?s=30) 
New to Julia? Check out [getting started with Julia](https://julialang.org/learning/) for documentation, worked examples or more tutorials.

## Running on your Computer
Wanna run this on your machine? You'll need to install Julia and Pluto.jl (Think Jupyter, but reactive), first. Instructions for both are here: [https://plutojl.org/#install](https://plutojl.org/#install)

## What's Pluto?
It's a reactive (like a spreadsheet), reproducable (yay, science), programming environment for Julia. Go check it out: [https://plutojl.org/](https://plutojl.org/)
"""

# ╔═╡ a8978a4a-01f2-445c-87fd-15c545af84c4
TableOfContents()
...
manifest_format = "2.0"
project_hash = "2a453e74b5d7ed6b3326e497ac7c81c276dc3186"
